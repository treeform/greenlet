...
static int _init_complete = 0;

static void _greenlet_tls_init(void)
{
    greenlet_t *root;
    ...
    if (pthread_key_create(&_current_greenlet, NULL) != 0)
        return;
    root = calloc(1, sizeof(greenlet_t));
    if (root != NULL)
    {
...
{
    pthread_once(&_tls_init_once, _greenlet_tls_init);
    if (!_init_complete)
        return NULL;
    return pthread_getspecific(_root_greenlet);
}
...
{
    printf("munmap %x %x\n", stack, size);
    int err = munmap(stack, size);
    printf("done munmap %x\n", err);
}
